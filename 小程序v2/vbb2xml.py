...
def vbb_anno2dict(vbb_file, cam_id):
    ...
    for frame_id, obj in enumerate(objLists):
        if len(obj) > 0:
            frame_name = "set00" + "_" + str(filename) + "_" + str(frame_id + 1) + ".jpg"
...
def parse_anno_file(vbb_inputdir, vbb_outputdir):
    ...
    for sub_dir in sub_dirs:
        ...
        # 获取某一个子set下面的所有vbb文件
        vbb_files = []
        vbb_files.append(vbb_inputdir +"\\"+cam_id)
...
def main():
    vbb_inputdir = "D:\\works\\软件项目管理\\小程序v1\\数据集\\labels"
    vbb_outputdir = "D:\\works\\软件项目管理\\小程序v1\\VOCdevkit\\Annotations"
